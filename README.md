...
- **`.cambium/`:** el método vendorizado — `CHARTER.md`, tu `REGLAS.md` y un `VERSION` con el sello **y la procedencia** (p. ej. `Cambium Charter v0.5` + `upstream: github.com/zigiella/cambium-charter`, para que actualizar con §IX resuelva la fuente solo). Opcional: `memoria/` si la coordinadora versiona su memoria.
...
Desciende del oficio **Cambium**. Textos bajo **CC-BY-4.0**; código y plantillas bajo **MIT** (ver `LICENSE`). Versión sellada en `.cambium/VERSION` (**v0.5**).
