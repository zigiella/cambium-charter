...
**Repo del proyecto:** `<REPO_URL>` — clónalo (o haz `git pull`) y trabaja SIEMPRE contra él.

**Adopta el método:** lee `CHARTER.md` en el repo (o https://github.com/zigiella/cambium-charter). En corto: el repo es la verdad · quédate en tu jurisdicción (fuera, handoff por escrito, no edites) · el PR es tu entrega · si algo falla, no inventes y el "no" es válido. Firma cada commit identity-inline: `git -c user.name="<Nombre>" -c user.email="<nombre>@<proyecto>.local" commit …`.
...
**Reglas que no se rompen:** `REGLAS.md` en el repo.
