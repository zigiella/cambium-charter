...
1. **Instala el método EN el repo del proyecto** (esto primero; no basta con leerlo), de forma ORDENADA para no ensuciar la raíz:
   ...
   - **El resto del método, en `.cambium/`:** `CHARTER.md`, `REGLAS.md` (innegociables del proyecto, del brief) y `VERSION` (el sello del kit + su procedencia, p. ej. `Cambium Charter v0.4` y `upstream: github.com/zigiella/cambium-charter`).
